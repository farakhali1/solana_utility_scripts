...
def get_block_for_slot(rpc_url, slot, rpc_client:Client, db, leader_identity):
    ...
    while attempt < max_retries:
        try:
            ...
            attempt += 1
            logger.error(
                f"Error in RPC for slot {slot}, retry attempt {attempt} {e}"
            )
...
def process_slots(args, db):
    try:
        ...
        with open(f"{args.start_slot}-metrics.csv", mode="w", newline="") as file:
            ...
            for each_slot in slots:
                ...
                result = get_block_for_slot(args.rpc_url, each_slot, rpc_client, db, leader_identity)
...
def main():
    ...
    parser.add_argument(
        "--count", type=int, default=4, help="Number of slots to process starting from  `--start-slot`, default is 4."
    )
    parser.add_argument(
        "--req_per_sec",
        type=int,
